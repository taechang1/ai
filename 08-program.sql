...
begin
  ...
  select*from usertbl;
  select*from vuser;
  -- 뷰 사용하는 이유 1.보안 2 복잡한 퀴리문 단순화 시키기
  use sqldb;
...
where `user id`='KBS' order by `product name`;-- `백터 ~키와 같이 있는것
...
as
select
u.name as 'name',u.addr'addr',concat_ws('-',u.mobile1,u.moblie2)
tel from usertbl u
inner join buytbl b
on u.userid=b.userid;
...
create or replace view v1 -- v1뷰가 있으면 고칙 ㅇ벗으면 새로 만들고
as
...
insert into cUsertbl (userid,name,birthyear,addr)
    ...
    desc v2;
    -- 뷰 수정 테이블이 수정될까?alter
    select*from v2;
    select*from  cUsertbl;
    -- 홍길동 birthyear 2019수정하기
    ...
    -- -----------
    -- 테이블 스페이스 : 물리적 공간
    -- DBA 데이터 베이스 관리 -- 서버 계정 공간 할당
    -- ------------
    -- 인데스
    -- 자료를 찾을 때 색인표 붙여 두는것
    -- 목적 : 빠르게 검색하려고
    -- B-Tree
    -- 장점:검색 속도 빠르다 항상 그런것은 아니다 시스템 성능이 향상된다
--     단점 추가적인 공간이 필요해 진다 대략 데이터 베이스 전체 용량의 10가 필요하다
--     처음 인데스르 생성하는데 시간이 많이 소요된다
--     데이터 변경직업이 자주 일어나는 경우 오히려 성능이 나빠질수 있다
...
-- 스토어드 프로그램 
-- 고급 sQl 문 상위 레벨
-- 종류 1스토어드 프로시저 2 스토어드 함수 3커서 4 트리거
/* 1. 스토어드프로시저 형식
delimiter $$
create procedure 이름 (입력 또는 출력 파라미터(매개 변수))
begin
end$$
delimiter;
call 이름(실제값들);
...
delimiter ;
  ...
  delimiter ;
  call p3(1973,170);
  -- testTbl 안에 데이터르 삽입하고 검색결과를 출력하는 프로시저
 ...
 select count(id) into outVar from tesTbl;
  end $$
  delimiter ;
