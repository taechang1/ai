...
#해설 온도와 기압은 비례관계이고 강한 상관 관계이다
#0~250도까지는 동일하지만 250도 이상이 되면 급격하게 증가한다
...
#즉 교육을 많이 받으면 수입 증가함을 알수 있다

# 6. 다음은 10일간의 습도와 자전거 대여량을 조사한 자료이다. 산점도와 상관계수를 구하고, 습도와 자전거 대여량 사이에 어떤 상관관계가 있는지 설명하시오.
hy<-c(56,67,89,91,54,41,46,71,80,92)
cycle.rent<-c(92510,74801,27168,33929,89807,92045,75328,48756,21875)
...
dim(state.x77)
cor(state.x77)
cor(state.x77)[2,]#결과 보면 hs grad가 상관관계 제일 높다 
#ii<-state.x77[2,]
#hh<-state.x77[,6]
#plot(ii,hh)
...
#9. 다음은 2014년 4분기부터 2017년 3분기까지 남녀 경제활동참가율을 나타낸 통계자료이다. 연도를 x축으로 하고 남녀를 각각 다른 선과 색으로 표시하는 그래프를 작성하시오
...
plot(year,male,type = 'b',col='blue',main = '남녀 경제 활동참가율',ylim = c(50,75))
...
head(Orange$Tree)
#위에 leves로 보니깐 5까지 되어 있다 즉 5개의 종류이다 색을 5개 지정해야하한다
col<-c('red','orange','green','blue','purple')
plot(Orange$age,Orange$ageOrange$age)
#12. mlbench 패키지에서 제공하는 Glass 데이터셋에 대해 다음의 문제를 해결하기 위한 R 코드를 작성하시오.
install.packages('mble')
...
colors<-c('#ff0000','#112255','#88ee11','#0088aa',
          '#00aa11','#00aaff','#2222ff','#99aa00',
          '#5588dd','#ffbb00')
plot(myds.notType,col=colors)
...
years<- 1875:1972
hlebels<-as.vector(LakeHuron)
plot(years,hlebels,type='b',col='blue',main = 'LakeHuron',ylab = '수위')
...
#1949년 정보 모두 추줄하기 시계열 객체명 시작년도 ,c시작년도 미자막달
...
plot(months,deaths.1973,ylim=c(6500,12000),type='b',
     ...
     ylab='사망자수',xlab='월')
lines( months, deaths.1975, type='b', col='green')
