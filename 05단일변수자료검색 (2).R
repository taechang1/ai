...
diff(range(m))
hist(m)
hist(mydata,main = '함수',xlab ='점수',ylab ='갯수',border ='blue',
...
head(iris)
unique(iris$Species)
#붓꽃의 종류 species별로 값(petal.lenght꽃잎길이)로 boxplot 그리기
# 형식 boxplot(그리려는 열값~ 그룹화할 열이름 ,data 데이터셋명)
...
idx<-Orange$Tree!=2 
idx
tree.2d<- Orange$circumference[idx]#idx 값이 true면 보이게 fasle 안보이게
mean(tree.2d)
median(tree.2d)
